...
SQL_SYSTEM = dedent(f"""
    ...
    ALWAYS use GROUP BY. NEVER use UNION ALL for group comparisons.
    NEVER filter in the WHERE clause on the dimension being measured as a percentage.
    The WHERE clause must not pre-filter rows to only matching cases, that makes
    percentages trivially 100%.
    Correct pattern for "what % of verified vs non-verified are CC?":
        SELECT is_verified,
                COUNT(*) AS total,
    ...
    The WHERE clause here should be empty (or only filter unrelated dimensions).
    Do NOT add WHERE warnings IS NULL or WHERE is_verified = 1 — those destroy the denominator.
...
def print_results(cols, rows, model, question):
    if not rows:
        print("DB Assistant: No results found.\n")
        return
...
def main():
    ...
    while True:
        ...
        if not sql:
            print(f"\DB Assistant: {sql_reply}\n")
            continue
        if not is_safe(sql):
